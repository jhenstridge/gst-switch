...
gst_switch_mdns_publish_new (GstSwitchServer * server, const char *service_name)
{
  ...
  GstSwitchMdnsPublish *self =
      g_object_new (GST_TYPE_SWITCH_MDNS_PUBLISH, NULL);
  if (!self) {
    return NULL;
  }
...
create_services (GstSwitchMdnsPublish * self)
{
  ...
  if (!self->group) {
    ...
    if (!self->group) {
      g_warning ("avahi_entry_group_new() failed: %s",
  ...
  if (avahi_entry_group_is_empty (self->group)) {
    ...
    caps_record =
        g_strconcat ("caps=", gst_switch_server_get_video_caps_str (), NULL);
    error =
        avahi_entry_group_add_service (self->group, AVAHI_IF_UNSPEC,
        AVAHI_PROTO_UNSPEC, 0, self->service_name, "_gstswitch-video._tcp",
        NULL, NULL, self->server->video_acceptor_port, version_record,
        caps_record, NULL);
    g_free (caps_record);
    if (error == AVAHI_ERR_COLLISION) {
      goto collision;
    } else if (error != 0) {
      g_warning ("Failed to add _gstswitch-video._tcp service: %s",
    ...
    /* Publish the video service */
    caps_record =
        g_strconcat ("caps=", gst_switch_server_get_audio_caps_str (), NULL);
    error =
        avahi_entry_group_add_service (self->group, AVAHI_IF_UNSPEC,
        AVAHI_PROTO_UNSPEC, 0, self->service_name, "_gstswitch-audio._tcp",
        NULL, NULL, self->server->audio_acceptor_port, version_record,
        caps_record, NULL);
    g_free (caps_record);
    if (error == AVAHI_ERR_COLLISION) {
      goto collision;
    } else if (error != 0) {
      g_warning ("Failed to add _gstswitch-audio._tcp service: %s",
          avahi_strerror (error));
      goto fail;
    }
...
static void
group_callback (AvahiEntryGroup * group, AvahiEntryGroupState state,
    void *user_data)
{
...
client_callback (AvahiClient * client, AvahiClientState state, void *user_data)
{
  ...
  switch (state) {
    ...
    case AVAHI_CLIENT_FAILURE:
      g_warning ("Avahi client failed: %s",
